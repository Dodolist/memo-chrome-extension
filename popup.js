...
memoTextarea.addEventListener("keydown", function (e) {
  console.log(memoTextarea.selectionStart);
  console.log(memoTextarea.value[memoTextarea.selectionStart-1]);
  setTimeout(() => {
    console.log(memoTextarea.value[memoTextarea.selectionStart]);
  }, 10);
});
...
function clearMemo() {
  const memoTextarea = document.querySelector(".memo");
  const blackScreen = document.querySelector(".black-screen");
  const fullScreen = document.querySelector(".full-screen");6
  const wrap = document.querySelector(".wrap");
  const bottombar = document.querySelector(".bottom");
  // 메모를 로컬 스토리지에서 삭제
